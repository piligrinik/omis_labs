...
class ParcelEditView(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # Первая колонка
        label0 = tk.Label(self, text="Редактирование посылки", bg='LightSteelBlue1', font=("Segoe UI Semibold", 20),foreground='RoyalBlue4')
        label0.place( y=10, x=200)
        label1 = tk.Label(self, text="Трек-номер послыки", bg='LightSteelBlue1')
        label1.place(x=10, y=60)
        entry1 = tk.Entry(self, width=50, state=tk.DISABLED)
        ...
        entry2 = tk.Entry(self, width=50, state=tk.DISABLED)
        ...
        entry3 = DateEntry(self, width=50, date_pattern='dd.MM.yyyy', state=tk.DISABLED)
        ...
        self.add_button = ttk.Button(self, text="Редактировать посылку", style='TButton', command=self.edit_parcel)
        self.add_button.place(x=200, y=300, width=340, height=40)

    def edit_parcel(self):
        messagebox.showinfo("Уведомление", "Посылка успешно редактирована!")
        self.destroy()
